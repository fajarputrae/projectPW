...
class m_field_search extends CI_Model{
	...
	public function get_results($search_term)
    {
        ...
        $this->db->select('f.id, f.name AS f_name, f.address, f.open_hour, f.close_hour, f.price, f.contact, f.img, c.category_id, c.name AS c_name, c.category_desc, c.img AS c_img');
